...
class DatabaseManager:
    ...
    def add_pet(self, name: str, face_embedding: Optional[str] = None) -> int:
        """
        Add a new pet to the database.
        
        Args:
            name: Pet's name
            face_embedding: Serialized face embedding data
        ...
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT INTO pets (name, face_embedding, updated_at)
                    VALUES (?, ?, CURRENT_TIMESTAMP)
                """, (name, face_embedding))
                conn.commit()
                pet_id = cursor.lastrowid
                logger.info(f"Added pet '{name}' with ID {pet_id}")
                return pet_id
    ...
    def get_events(self, pet_id: Optional[int] = None, event_type: Optional[str] = None,
    ...
    def set_alert_config(self, user_id: str, no_event_threshold: int = 60,
                        alert_enabled: bool = True, 
                        notification_methods: Optional[List[str]] = None) -> None:
        """
        ...
        """
        try:
            methods_json = json.dumps(notification_methods) if notification_methods else None
            ...
            with self.get_connection() as conn:
                ...
                if row:
                    ...
                    if config['notification_methods']:
                        config['notification_methods'] = json.loads(config['notification_methods'])
                    return config
                return None
        except sqlite3.Error as e:
